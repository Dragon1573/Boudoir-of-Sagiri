...
**旭日图** 能够可视化从根到叶径向外扩的分层数据。与 [冰柱图](https://plotly.com/python/icicle-charts/) 和 [树状图](/visualization/plotly/treemaps) 类似，分层结构由`labels`（`px.icircle`则叫做`names`）和`parents`定义。图表的根从圆心出发，其子项目则被添加在外圈上。
